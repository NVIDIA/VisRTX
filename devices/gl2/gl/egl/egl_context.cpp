...
// eglContext definitions /////////////////////////////////////////////////////

eglContext::eglContext(
    ANARIDevice device, EGLDisplay display, EGLenum api, EGLint debug)
    : GLContextInterface(device), display(display), api(api), debug(debug)
{}
...
void eglContext::init()
{
  ...
  anariDeviceReportStatus(
      d, ANARI_SEVERITY_INFO, ANARI_STATUS_NO_ERROR, "[OpenGL] using EGL");

  // NOTE: If there is no display injected attempt to use the platform one
  ...
  if (display == nullptr) {
    ...
    this->display =
        eglGetPlatformDisplayEXT(EGL_PLATFORM_DEVICE_EXT, devices[0], 0);
  }
  ...
  if (api == EGL_OPENGL_ES_API) {
    static const EGLint contextAttribs[] = {EGL_CONTEXT_OPENGL_DEBUG,
        debug,
    ...
    context = eglCreateContext(
        this->display, EGL_NO_CONFIG_KHR, EGL_NO_CONTEXT, contextAttribs);
