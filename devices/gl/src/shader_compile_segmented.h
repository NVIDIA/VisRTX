...
template<typename T>
inline static GLuint compile_shader_segmented(T &gl, GLenum type, const char *const *source) {
    GLint status;
    ...
    if (status == GL_FALSE) {
        ...
        std::fprintf(stderr, "[GLSL] %s shader compilation error %s\n", shader_name(type), log.data());                
...
inline static GLuint shader_build_graphics_segmented(T &gl,
    ...
    if (vertex_source != nullptr)
        shaders[count++] = compile_shader_segmented(gl, GL_VERTEX_SHADER, vertex_source);
    if (control_source != nullptr)
        shaders[count++] = compile_shader_segmented(gl, GL_TESS_CONTROL_SHADER, control_source);
    if (eval_source != nullptr)
        shaders[count++] = compile_shader_segmented(gl, GL_TESS_EVALUATION_SHADER, eval_source);
    if (geometry_source != nullptr)
        shaders[count++] = compile_shader_segmented(gl, GL_GEOMETRY_SHADER, geometry_source);
    if (fragment_source != nullptr)
        shaders[count++] = compile_shader_segmented(gl, GL_FRAGMENT_SHADER, fragment_source);
...
template<typename T>
inline static GLuint shader_build_compute_segmented(T &gl, const char *const *compute_source) {
    GLuint shaders[1];
    int count = 0;
    if (compute_source != nullptr)
        shaders[count++] = compile_shader_segmented(gl, GL_COMPUTE_SHADER, compute_source);
