      ...
      m_defaultAndBodyTextureDescriptors(defaultAndBodyTextureDescs)
{
  for (auto i = 0ull, num_params = compiledMaterial->get_parameter_count();
    ...
    case mi::neuraylib::IValue::VK_VECTOR: {
      auto ivector = make_handle(argument->get_interface<mi::neuraylib::IValue_vector>());
      ...
      switch (size) {
        case 2: {
          switch (element_type->get_kind()) {
            ...
            default: {
              core->logMessage(mi::base::MESSAGE_SEVERITY_WARNING, "Unsupport vector type {},  ignoring", int(element_type->get_kind()));
              break;
            }
          }
          break;
        ...
        default: {
          core->logMessage(mi::base::MESSAGE_SEVERITY_WARNING, "Unsupport vector size {}, ignoring", size);
          break;
        }
      }
      break;
