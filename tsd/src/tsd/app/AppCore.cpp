...
anari::Device ANARIState::loadDevice(const std::string &libraryName)
{
  if (libraryName.empty() || libraryName == "{none}")
    return nullptr;

  anari::Device dev = this->loadedDevices[libraryName];
  if (dev) {
  ...
  this->loadedDeviceExtensions[libraryName] =
  ...
  this->loadedDevices[libraryName] = dev;
  anari::retain(dev, dev);

  return dev;
}

const anari::Extensions *ANARIState::loadDeviceExtensions(
    const std::string &libName)
{
  ...
  if (!d)
    return nullptr;
  anari::release(d, d);
  return &this->loadedDeviceExtensions[libName];
}

tsd::RenderIndex *ANARIState::acquireRenderIndex(anari::Device d)
{
  auto &liveIdx = this->rIdxs[d];
  if (liveIdx.refCount == 0) {
#if 1
    liveIdx.idx =
        this->delegate.emplace<tsd::RenderIndexAllLayers>(this->tsd.ctx, d);
#else
    liveIdx.idx =
        this->delegate.emplace<tsd::RenderIndexFlatRegistry>(this->tsd.ctx, d);
#endif
...
void ANARIState::releaseRenderIndex(anari::Device d)
{
  auto &liveIdx = this->rIdxs[d];
  if (liveIdx.refCount == 0)
    return;
  else if (liveIdx.refCount == 1)
    this->delegate.erase(liveIdx.idx);
  liveIdx.refCount--;
}

void ANARIState::releaseAllDevices()
{
