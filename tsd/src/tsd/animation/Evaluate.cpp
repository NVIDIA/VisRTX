...
static TimeSample findTimeSample(const float *times, size_t count, float t)
{
  ...
  if (count == 1 || t <= times[0])
    return {0, 0, 0.f};
  if (t >= times[count - 1])
    return {count - 1, count - 1, 0.f};

  auto it = std::upper_bound(times, times + count, t);
  auto hi = static_cast<size_t>(it - times);
  auto lo = hi - 1;
    ...
    const std::vector<Animation> &animations, float currentTime)
{
  ...
  for (const auto &anim : animations) {
    for (const auto &binding : anim.bindings) {
      ...
      auto sample = findTimeSample(
          binding.timeBase.data(), binding.timeBase.size(), currentTime);
    ...
    for (const auto &tb : anim.transforms) {
      ...
      auto sample = findTimeSample(
          tb.timeBase.data(), tb.timeBase.size(), currentTime);
      ...
      if (sample.lo == sample.hi) {
        ...
        rot = slerp(tb.rotation[sample.lo], tb.rotation[sample.hi], sample.alpha);
        t = lerpV3(tb.translation[sample.lo], tb.translation[sample.hi], sample.alpha);
