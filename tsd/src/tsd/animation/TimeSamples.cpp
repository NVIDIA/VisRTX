...
TimeSamples::TimeSamples(anari::DataType elementType, size_t items)
    : m_type(ANARI_ARRAY1D), m_elementType(elementType), m_size(items)
{
  if (isEmpty()) {
...
anari::DataType TimeSamples::type() const
{
  return m_type;
}
...
TimeSamples::TimeSamples(const TimeSamples &o)
    : m_type(o.m_type), m_elementType(o.m_elementType), m_size(o.m_size)
{
