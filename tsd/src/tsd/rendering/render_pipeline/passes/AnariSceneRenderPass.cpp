...
AnariSceneRenderPass::AnariSceneRenderPass(anari::Device d) : m_device(d)
{
  ...
  if (m_deviceSupportsCUDAFrames)
    tsd::core::logStatus("[render_pipeline] using CUDA-mapped fb channels");
  else
    tsd::core::logStatus("[render_pipeline] using host-mapped fb channels");
}
...
void AnariSceneRenderPass::setEnableIDs(bool on)
{
  ...
  if (on) {
    tsd::core::logInfo("[render_pipeline] enabling objectId frame channel");
    ...
    tsd::core::logInfo("[render_pipeline] disabling objectId frame channel");
