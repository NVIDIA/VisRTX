...
bool CopyToSDLTexturePass::checkGLInterop() const
{
  ...
  if (err != cudaSuccess) {
    tsd::core::logWarning("[render_pipeline] failed to get CUDA GL devices");
    return false;
  }
  ...
  if (numDevices > 0) {
    ...
    for (unsigned int i = 0; i < numDevices; ++i) {
      if (currentDevice == cudaDevices[i]) {
        tsd::core::logStatus(
            "[render_pipeline] using CUDA-GL interop via SDL3");
        return true;
      }
    }
  }
  ...
  tsd::core::logWarning(
      "[render_pipeline] unable to use CUDA-GL interop via SDL3");
  return false;
}
...
void CopyToSDLTexturePass::updateSize()
{
  ...
  if (texID > 0) {
    ...
    tsd::core::logWarning(
        "[render_pipeline] could not get SDL texture number!");
  }
#endif
}
